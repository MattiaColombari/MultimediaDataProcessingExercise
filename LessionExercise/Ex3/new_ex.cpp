...
namespace mdp {         // Definiamo il nostro namespace.

    template<typename T>
    class vector {          // é equivalente a una struct.
        private:            // Elementi privati alla classe.
        T *nums_;
        int n_;
        int capacity_;

        public:             // Elementi pubblici accessibili dall'esterno.
        ...
        /*
            Non funziona, controlla il motivo.
        ...
        */
        ...
        // Questo è il modo corretto di scrivere la swap.
        friend void swap(vector& left, vector& right){
            // Comando per fede.
            using std::swap;

            // Using rende disognoibile la funzione swap nel namespace globale.
            // Tipo la "import" in Python, che se non la stessa cosa.

            /*
                Arguments Dependent Lookup, ADL
            */
...
mdp::vector<esempio> crea_mio(){
    mdp::vector<esempio> out;
    out.push_back(6);
    out.push_back(-9);
    out.push_back(23);
    return out;
...
int main(int argc, char *argv[]){
    using namespace mdp;        // Usalo solo una volta nelle funzioni, altrimenti fai conflitti
                                // con altri namespace.

    // I namespace ci permetted i separare classi, in modo da separare i vettori della
    // std dai nostri.
    vector<esempio> mio;
    mio = crea_mio();
